...
export function DemoChat() {
  return (
    <section className="overflow-hidden rounded-[2rem] border border-[#d4cdc2] bg-[#efeae2] shadow-glow">
      ...
      <div className="space-y-2.5 bg-[#efeae2] px-3 py-4 sm:px-4">
        {conversation.map((item, index) => {
          ...
          return (
            <div key={item.id} className={`max-w-[88%] ${isUser ? "ml-auto" : ""}`}>
              ...
              <article
                className={`animate-[bubbleIn_.3s_ease-out_both] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${
                  isUser
                    ? "ml-auto rounded-br-md bg-[#DCF8C6] text-[#1f3120]"
                    : "rounded-tl-md bg-white text-slate-800"
                }`}
                style={{ animationDelay: `${index * 85 + 50}ms` }}
              >
                <p>{item.message}</p>
